#!/bin/bash
# =============================================================================
# Madeena IAM — Hybrid Local Development Setup
# =============================================================================
...
#   • Infrastructure Layer: MySQL 8.4 runs inside Docker (port-forwarded)
#
# USAGE:
...
#
# PREREQUISITES:
#   PHP 8.4, Composer, Node.js 18+, NPM, Docker Desktop (WSL integration on)
# =============================================================================
set -euo pipefail

# ─── CONFIGURATION ────────────────────────────────────────────────────────────
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP_NAME="madeena-iam"
...
Architecture:
  ...
  MySQL 8.4 runs in Docker for isolation and easy teardown.
  Connection: PHP → 127.0.0.1:3310 → Docker MySQL (port-forwarded)

Prerequisites:
  • PHP 8.4 with extensions: pdo_mysql mbstring xml gd zip intl bcmath pcntl
  • Composer 2.x
  • Node.js 18+ and NPM
...
# ─── HEADER ───────────────────────────────────────────────────────────────────
echo ""
echo -e "${BOLD}🚀  Madeena IAM — Hybrid Local Development Setup${RESET}"
echo "────────────────────────────────────────────────"
if [[ "${FRESH}" == "true" ]]; then
...
if ! docker image inspect mysql:8.4 &>/dev/null; then
    info "Pulling mysql:8.4 image (first time only)…"
...
echo ""
echo -e "${BOLD}${GREEN}══════════════════════════════════════════════════════${RESET}"
echo -e "${BOLD}${GREEN}  ✅  Madeena IAM local environment ready!${RESET}"
echo -e "${BOLD}${GREEN}══════════════════════════════════════════════════════${RESET}"
echo ""
