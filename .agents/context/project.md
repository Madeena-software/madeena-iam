...
authority_note: This document provides supporting orientation context for AI agents and developers. It is not primary product or delivery authority. Refer to canonical specifications and observed repository evidence for primary truth.
---
...
1. **OAuth2 Authorization & SSO Surface (`/oauth/*`, `/login`, `/register`)**:
   - Implements the OAuth2 Authorization Code grant flow with PKCE support.
...
2. **Direct API Surface (`/api/v1/*`)**:
   - `POST /api/v1/auth/login`: Direct server-to-server validation (validates client secret, user credentials, client access mapping, and issues Passport access tokens).
...
|---|---|---|
| Product Requirements & Behavior | [`docs/madeena_iam_prd.md`](file:///var/www/madeena-iam/docs/madeena_iam_prd.md) | Defines product intent, user personas, functional requirements, and application flows. |
| AI Delivery Contract & Governance | [`.agents/AGENTS.md`](file:///var/www/madeena-iam/.agents/AGENTS.md) | Defines agent roles (Planner, Executor, Reviewer), task boundaries, and execution rules. |
| Software Delivery Lifecycle | [`.agents/software-workflow.md`](file:///var/www/madeena-iam/.agents/software-workflow.md) | Defines the formal delivery protocol, quality gates (B0–G10), and traceability requirements. |
| Production Topology & Invariants | [`docs/production-architecture.md`](file:///var/www/madeena-iam/docs/production-architecture.md) | Documents Swarm services, network topology, port bindings, and operational constraints. |
...
The following operational invariants are enforced in production and must not be altered without explicit architecture planning:
...
2. **Nginx Update & Rollback Order (`stop-first`)**:
   - The `nginx` service MUST use `update_config.order: stop-first` and `rollback_config.order: stop-first`.
   - **Reason**: In a single-node manager topology with a host-mode port, `start-first` causes the new replica to fail with `address already in use` (port 8012 collision) before the old task is stopped. `stop-first` ensures the old task vacates the port before the new task binds it.
...
4. **Overlay Network Scope**:
   - Services communicate over the stack-scoped overlay network `madeena-iam_network` (`driver: overlay`, `attachable: true`). Nginx proxies FastCGI requests to `madeena-iam_app:9000`.

5. **Deployment Control Path**:
   - Production deployments are executed exclusively via GitHub Actions (`.github/workflows/deploy-swarm.yml`) running on the self-hosted production runner.
...
- **Accepted Implementation Baseline**: `be39763b73ca7ebbc59fecc4ab0be0aad78bc4a2` (`fix(swarm): set nginx update and rollback order to stop-first for host-mode port`).
- **Incident Resolution**: The recent HTTP 502 deployment regression caused by host-port binding conflicts during Swarm rollouts is resolved. Post-deploy verification (run `32376272592`) and read-only diagnostic probing (run `32376637733`) confirmed healthy service convergence and `/up` endpoint availability.
- **Active Task Scope**: Documentation baseline reconciliation (re-establishing clear separation between PRD, production architecture, agent context, and developer README). No application runtime or schema modifications are authorized in this delivery cycle.
